...
function resetEntireGame(advanceHole = false) {
    ...
    const initialTeeScale = isMobileOnStart ? 0.65 : 0.65; /* Matches your custom computer desktop size */
    ...
    if (physics.isMoving) {
        if (!wasMoving) {
            ...
            if (teeBox && teeBox.visible) {
                window.shotStartScale = isCurrentMobile ? 0.65 : 0.65;
        ...
        if (teeBox && teeBox.visible) {
            ...
            ballTargetScale = isMobile ? 0.65 : 0.65; // Change first number for mobile, second for desktop
        } else if (onGreen) {
